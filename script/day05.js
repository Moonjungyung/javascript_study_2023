...
//버튼 클릭 시 input 현재값을 파악 후 결과 출력하기
resultBtn.addEventListener('click',function(){
    userInputNumber = Number(userInput.value);
    console.log(`userInputNumber type = ${typeof(userInputNumber)}`);
    console.log(`userInputNumber value = ${userInputNumber}`);
    //버튼 클릭 시 input 값에 +1한 결과 콘솔 출력
    console.log(userInputNumber+1) //x 10+10=1010
...
const test = document.querySelector('#test'); //버튼
...
//나머지연산자(-,*,/,%) 는 문자를 암시적형변환으로 자동 변환해
//숫자 데이터로 변경하여 문자*문자=숫자 결과로 보여준다
